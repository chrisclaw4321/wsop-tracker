      ...
      {selectedTournamentDetail && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl border-4 border-blue-400 shadow-2xl max-w-2xl w-full max-h-96 overflow-y-auto">
            ...
            <div className="flex items-start justify-between p-3 bg-gradient-to-r from-blue-100 to-cyan-100 border-b-4 border-blue-400 sticky top-0">
              <h2 className="text-sm font-bold text-blue-900 flex-1">{selectedTournamentDetail.name}</h2>
              <button
                onClick={() => setSelectedTournamentDetail(null)}
                className="ml-2 p-1 bg-red-400 hover:bg-red-600 text-white rounded-lg transition font-bold flex-shrink-0"
                title="Close details"
              >
                <X className="w-3 h-3" />
              </button>
            ...
            <div className="p-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                {selectedTournamentDetail.eventNum && (
                  <span className="px-2 py-0.5 bg-yellow-200 text-yellow-900 rounded-lg text-xs font-bold border-2 border-yellow-400">
                    Event #{selectedTournamentDetail.eventNum}
                  </span>
                )}
                {selectedTournamentDetail.eventType === 'satellite' && (
                  <span className="px-2 py-0.5 bg-amber-200 text-amber-900 rounded-lg text-xs font-bold border-2 border-amber-400">
                    SAT
                  </span>
                )}
                {selectedTournamentDetail.eventType === 'side' && (
                  <span className="px-2 py-0.5 bg-teal-200 text-teal-900 rounded-lg text-xs font-bold border-2 border-teal-400">
                    SIDE
                  </span>
                )}
                <span className={`px-2 py-0.5 rounded-lg text-xs font-bold ${getFormatBadgeColor(selectedTournamentDetail.format)}`}>
                  {getFormatLabel(selectedTournamentDetail.format)}
                </span>
              </div>

              {/* Date & Time */}
              <div className="bg-yellow-100 px-2 py-1.5 rounded-lg border-2 border-yellow-400 flex items-center gap-1">
                <Clock className="w-3 h-3 text-yellow-700" />
                <div>
                  <p className="text-xs text-yellow-700 font-bold">Start Time</p>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-green-100 px-2 py-1.5 rounded-lg border-2 border-green-400">
                  <p className="text-xs text-green-700 font-bold">Buy-in</p>
                  <p className="text-sm font-black text-green-900">{formatCurrency(selectedTournamentDetail.buyIn)}</p>
                </div>
                <div className="bg-cyan-100 px-2 py-1.5 rounded-lg border-2 border-cyan-400">
                  <p className="text-xs text-cyan-700 font-bold">Rake</p>
                  <p className="text-sm font-black text-cyan-900">{formatCurrency(selectedTournamentDetail.rakeFee)}</p>
                </div>
              </div>

              {/* Blind Levels */}
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-indigo-50 px-2 py-1 rounded-lg border-2 border-indigo-300">
                  <p className="text-xs text-indigo-600 font-bold">Level</p>
                  <p className="text-xs text-indigo-900">{parseLevelLength(selectedTournamentDetail.blindLevels)}</p>
                </div>
                <div className="bg-orange-50 px-2 py-1 rounded-lg border-2 border-orange-300">
                  <p className="text-xs text-orange-600 font-bold">Blinds</p>
                  <p className="text-xs text-orange-900">{parseStartingBlinds(selectedTournamentDetail.blindLevels)}</p>
                </div>
                <div className="bg-emerald-50 px-2 py-1 rounded-lg border-2 border-emerald-300">
                  <p className="text-xs text-emerald-600 font-bold">Stack</p>
                  <p className="text-xs text-emerald-900">{formatStack(selectedTournamentDetail.startingStack)}</p>
                </div>
              </div>
